...
FFMPEG_MARKER="$FFMPEG_LIB/.build_complete"
...
if [ "$FORCE_FFMPEG" = true ]; then
  build_ffmpeg --clean
elif [ ! -f "$FFMPEG_MARKER" ] || [ ! -d "$FFMPEG_LIB/lib" ] || \
     [ ! -f "$FFMPEG_REVISION_FILE" ] || \
...
cp "$PROJECT_ROOT/THIRD_PARTY_NOTICES.md" "$OUT_LICENSES/THIRD_PARTY_NOTICES.md"
