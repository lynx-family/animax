...
/**
 * Enum for AnimaX layer bounds space.
 */
export enum AnimaXLayerBoundsSpace {
  /** Bounds in the parent layer's coordinate space. */
  Parent = 0,
  /** Bounds in the top-level/root coordinate space. */
  Root = 1,
}
...
/** Rendering became temporarily unavailable and will be recreated. */
export interface AnimaXRenderingUnavailableParam {
  reason: 'context-lost' | 'dimension-change';
}
...
export type AnimaXFrameAvailableEvent = AnimaXEvent<AnimaXParam>;
export type AnimaXRenderingUnavailableEvent = AnimaXEvent<AnimaXRenderingUnavailableParam>;
...
export interface AnimaXFontDescriptor {
  /** Logical family name used by AnimaX and Textra font matching. */
  family: string;
  /** Font file URL. TTF/OTF is expected by the wasm font loader. */
  url: string;
}

export interface AnimaXFontConfig {
  /** Family promoted to the configured default fallback font. */
  defaultFamily?: string;
  /** Fonts to fetch and register into the wasm font registry. */
...
export interface AnimaXViewProps {
  ...
  /**
   * Whether frame capture is enabled for this view. Set this before the view
   * initializes. The next rendered WebGL frame is retained for capture when
   * enabled.
   */
...
export interface AnimaXViewController {
  /**
   * Capture the currently presented WebGL frame.
   * The capture-frame attribute must be set before the view initializes.
   * Returns null when capture is disabled, unavailable, or using WebGPU.
   */
  captureFrame(type?: string, quality?: number): Promise<Blob | null>;
  /** Request a newly rendered frame and emit frameavailable when it is ready. */
   ...
   * This is an asynchronous operation, the result will be returned via callback.
   *
   * @param layerName The name of the layer to search for (must not be empty)
   * @param boundsSpace The coordinate space to use for the bounds calculation.
   * @param callback The callback to receive the bounds result
   *                 Parameters: (success: boolean, x: number, y: number, width: number, height: number)
   */
  getLayerBounds(
    layerName: string,
