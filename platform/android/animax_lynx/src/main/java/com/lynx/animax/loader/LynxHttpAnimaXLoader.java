...
public class LynxHttpAnimaXLoader implements IAnimaXLoader {
  ...
  public void load(IAnimaXLoaderRequest request, IAnimaXLoaderCompletionHandler completionHandler) {
    // Handle image loading with Fresco if request contains image info
  ...
  /**
   * Handle image loading request with Fresco if applicable
  ...
  private boolean handleImageRequest(
    ...
    FrescoUtil.tryHandleLoaderRequestWithFresco(redirectedRequest, completionHandler);
    return true;
  }
  ...
  private boolean tryLoadWithGenericFetcher(
    ...
    fetcher.fetchResource(lynxRequest, new LynxResourceCallback<byte[]>() {
      ...
      public void onResponse(LynxResourceResponse<byte[]> response) {
        ...
        if (byteData != null && response.getState() == LynxResourceResponse.ResponseState.SUCCESS) {
          completionHandler.onComplete(AnimaXLoaderResponse.createByteArrayResponse(byteData));
        } else {
