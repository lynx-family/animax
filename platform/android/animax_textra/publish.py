...
""".replace("\n", " ")
    ...
    cmd = f"""./gradlew :AnimaXTextra:publish {property_param} '-PPUBLISH_INFO={publish_info}'"""
    print(f"execute command: {cmd}")
    result = os.system(cmd)
    if result != 0:
        result = 1
        sys.stderr.write(f"""ERROR: publish AnimaXTextra failed, params:{property_param}""")
        sys.stderr.flush()
    sys.exit(result)
