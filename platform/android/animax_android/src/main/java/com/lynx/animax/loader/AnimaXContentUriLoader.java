...
public class AnimaXContentUriLoader implements IAnimaXLoader {
  @Override
  public void load(IAnimaXLoaderRequest request, IAnimaXLoaderCompletionHandler completionHandler) {
    ...
    byte[] data = null;
    if (uri != null) {
      data = ResourceUtil.getByteArrayFromContentUri(uri);
    }
    ...
    if (data != null) {
      ...
      response = AnimaXLoaderResponse.createErrorResponse(
          new Throwable("load file failed, getByteArrayFromContentUri returned null."));
    }
  ...
  public AnimaXLoaderScheme getScheme() {
    return AnimaXLoaderScheme.CONTENT_URI;
  }
}
