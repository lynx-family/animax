...
std::unique_ptr<RepeaterModel> RepeaterParser::Parse(
  ...
  for (auto it = object.MemberBegin(); it != object.MemberEnd(); it++) {
    ...
    if (strcmp(key, "nm") == 0) {
      ...
      copies = AnimatableValueParser::ParseFloat(it->value, composition, false);
    } else if (strcmp(key, "o") == 0) {
      offset = AnimatableValueParser::ParseFloat(it->value, composition, false);
    } else if (strcmp(key, "tr") == 0) {
      transform = AnimatableTransformParser::Parse(it->value, composition);
    } else if (strcmp(key, "hd") == 0) {
      hidden = it->value.GetBool();
    }
  }

  return std::unique_ptr<RepeaterModel>(
      new RepeaterModel(std::move(name), std::move(copies), std::move(offset),
