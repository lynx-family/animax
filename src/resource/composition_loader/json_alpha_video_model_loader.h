...
#ifndef ANIMAX_SRC_RESOURCE_COMPOSITION_LOADER_JSON_ALPHA_VIDEO_MODEL_LOADER_H_
#define ANIMAX_SRC_RESOURCE_COMPOSITION_LOADER_JSON_ALPHA_VIDEO_MODEL_LOADER_H_
...
class JSONAlphaVideoModelLoader : public CompositionModelLoader {
 public:
  ...
  ~JSONAlphaVideoModelLoader() override = default;
  void Load(CompositionModelRequest request, CallbackType callback) override;

 private:
  explicit JSONAlphaVideoModelLoader(
      std::weak_ptr<ResourceLoaderListener> listener = {});
  static Ptr MakePipeline(
      Loader<ResourceRequest, ResourceResponse>::Ptr resource_loader,
      std::weak_ptr<JSONAlphaVideoModelLoader> weak_self);
  Ptr pipeline_{};
...
#endif  // ANIMAX_SRC_RESOURCE_COMPOSITION_LOADER_JSON_ALPHA_VIDEO_MODEL_LOADER_H_
