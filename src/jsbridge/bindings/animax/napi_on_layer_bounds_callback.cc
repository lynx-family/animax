...
// clang-format off
#include "jsbridge/bindings/animax/napi_on_layer_bounds_callback.h"
...
namespace lynx {
namespace animax {
const uint64_t kNapiOnLayerBoundsCallbackClassID = reinterpret_cast<uint64_t>(&kNapiOnLayerBoundsCallbackClassID);

NapiOnLayerBoundsCallback::NapiOnLayerBoundsCallback(Napi::Function callback) {
  ...
  if (storage == nullptr) {
    ...
    env.SetInstanceData(kNapiOnLayerBoundsCallbackClassID, storage, [](napi_env env, void* finalize_data,
...
Napi::Env NapiOnLayerBoundsCallback::Env(bool *valid) {
  if (valid != nullptr) {
    *valid = false;
  }
