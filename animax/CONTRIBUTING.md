...
First off, thank you for considering contributing to AnimaX!
We welcome you to join AnimaX Authors and become a member.
...
4. Once you have finished the necessary tests and verifications locally,
   ...
   We use the labels `[Required]` and `[Optional]` to differentiate them in the detailed explanation below:
    ...
    - **[Required]** The title must start with at least one label, and the first label must be one of the following:
      ...
      > - **`[Feature]`**: New features, new functions, or changes to existing features and functions.
      ...
      or architecture optimization; small-scale refactoring can be classified as Optimize).
      > - **`[Optimize]`**: Small-scale optimization of a certain feature or indicator,
      such as performance optimization, memory optimization, etc.
      > - **`[Infra]`**: Changes related to the compilation framework, CQ configuration, basic tools, etc.
      > - **`[Testing]`**: Modifications related to test cases and test frameworks.
      >
...
A Pull Request needs to be verified by the CI workflows and reviewed by the AnimaX authors before being merged.
...
(if this is your first PR for AnimaX, you'll see a pending approval on the PR discussion area after you submit the PR)
...
These tasks can also be executed individually via commands, for example, the `coding-style` task can be run with `git lynx check --checkers=coding-style`.
