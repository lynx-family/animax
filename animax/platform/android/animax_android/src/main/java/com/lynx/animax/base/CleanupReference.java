...
public class CleanupReference extends PhantomReference<Object> {
    ...
    public void run() {
      while (true) {
        try {
          ...
          if (DEBUG)
            AnimaXLog.d(TAG, "removed one ref from GC queue");
          ...
          AnimaXLog.e(TAG, "Queue remove exception:" + e.toString());
        }
      }
    }
  };
        ...
        try {
          ...
          switch (msg.what) {
            ...
            case REMOVE_REF:
              ...
              break;
            default:
              AnimaXLog.e(TAG, String.format("Bad message=%d", msg.what));
              break;
          }
  ...
  public CleanupReference(Object obj, Runnable cleanupTask, boolean cleanupOnUiThread) {
    super(obj, sGcQueue);
    if (DEBUG)
      AnimaXLog.d(TAG, "+++ CREATED ONE REF");
  ...
  public void cleanupNow() {
    if (!mCleanupOnUiThread) {
      // When mCleanupOnUiThread is set false, the cleanup task is not guaranteed to run in the UI
      // thread and will be executed in the calling thread.
  ...
  private void runCleanupTaskInternal(Set<CleanupReference> refs) {
    if (DEBUG)
      AnimaXLog.d(TAG, "runCleanupTaskInternal");
    if (!refs.remove(this)) {
      return;
    }
    ...
    if (cleanupTask != null) {
      if (DEBUG)
        AnimaXLog.i(TAG, "--- CLEANING ONE REF");
      cleanupTask.run();
    }
    clear();
  }
